...
namespace MobileAppTracking
{
    ...
    public class MATEventQueue
    {
        ...
        private const string SETTINGS_MATEVENTQUEUE_ATTEMPT_KEY = "mat_event_queue_attempt";
        ...
        protected internal int GetQueueSize()
        {
            ...
            if (GetLocalSetting(SETTINGS_MATEVENTQUEUESIZE_KEY) != null)
                return (int)GetLocalSetting(SETTINGS_MATEVENTQUEUESIZE_KEY);
        ...
        protected internal void AddToQueue(Object url, Object attempt)
        {
            lock (syncLock)
            {
                ...
                string eventQueueAttempt = SETTINGS_MATEVENTQUEUE_ATTEMPT_KEY + "_" + eventQueueSize.ToString();
        ...
        {
            SaveLocalSetting(SETTINGS_MATEVENTQUEUESIZE_KEY, GetQueueSize() - 1);
            string eventQueueKey = SETTINGS_MATEVENTQUEUE_KEY + "_" + key.ToString();
            string eventqueueAttemptKey = SETTINGS_MATEVENTQUEUE_ATTEMPT_KEY + "_" + key.ToString();
            ...
            lock (syncLock)
            {
                ...
                if (eventQueueSize > 0)
                {
                    for (int i = 0; i < eventQueueSize; i++)
                    {
                        ...
                        string eventQueueAttemptKey = SETTINGS_MATEVENTQUEUE_ATTEMPT_KEY + "_" + i.ToString();
                        if (parameters.localSettings.Values.ContainsKey(eventQueueKey))
                        {
                            ...
                            SaveLocalSetting(SETTINGS_MATEVENTQUEUESIZE_KEY, (int)parameters.localSettings.Values[SETTINGS_MATEVENTQUEUESIZE_KEY] - 1);
                        }
                    }
                }
            }
