...
- (NSDictionary*)pop
{
    ...
    DebugLog(@"TuneReqQuePart: pop: requestsCnt   = %lu", (unsigned long)self.requests.count);
    DebugLog(@"TuneReqQuePart: pop: loadedReqCnt  = %lu", (unsigned long)self.loadedRequestsCount);
    DebugLog(@"TuneReqQuePart: pop: shouldLoad    = %d", self.shouldLoadOnRequest);
    ...
    DebugLog(@"TuneReqQuePart: pop: remaining requests queue size = %lu", (unsigned long)requests_.count);
    
    DebugLog(@"TuneReqQuePart: pop: %@", requestData);
    return requestData;
}
...
- (BOOL)load
{
    ...
    if (!self.loaded)
    {
        ...
        if (fileData)
        {
            ...
            {
                DebugLog(@"ignore xml file, too big to parse = %d", (int)fileData.length);
            }
#endif
...
- (NSString*)serialize
{
    ...
    @synchronized( requests_ ) {
        for (NSDictionary * request in self.requests)
        {
            ...
            NSString *encodedUrl = [TuneXmlUtils Tunegtm_stringBySanitizingAndEscapingForXML:urlValue];
            
            NSString *encrypt = [request valueForKey:TUNE_KEY_DATA];
            NSString *encodedEncrypt = [TuneXmlUtils Tunegtm_stringBySanitizingAndEscapingForXML:encrypt];
            ...
            NSString *encodedJson = [TuneXmlUtils Tunegtm_stringBySanitizingAndEscapingForXML:jsonValue];
    ...
    DebugLog(@"TuneRequestsQueuePart serialize output = %@", strDescr);
...
- (NSString*)generateFileName
{
    ...
    DebugLog(@"TuneReqQuePart: generateFileName: file = %@", self.fileName);
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
    ...
    {
        DebugLog(@"TuneReqQuePart: parser: create object using: %@", attributeDict);
