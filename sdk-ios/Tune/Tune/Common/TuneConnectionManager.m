...
/********* TuneConnectionManager private interface ********/

@interface TuneConnectionManager()

@property (nonatomic, retain) TuneReachability * reachability;
...
/********* TuneConnectionManager implementation ********/

@implementation TuneConnectionManager
...
- (id)init
{
    ...
    if (self)
    {
        requestsQueue_ = [[TuneRequestsQueue alloc] init];
        ...
        DLog(@"TuneConnManager: init(): requestsQueue = %@", requestsQueue_);
        ...
        self.reachability = [TuneReachability reachabilityForInternetConnection];
        
        DLog(@"TuneConnManager: init(): reachability = %@", self.reachability);
...
- (void)dealloc
{
    ...
    DLog(@"TuneConnManager: dealloc: stop reachability notifier: self = %@, self.reachability = %@", self, self.reachability);
...
- (void)handleNetworkChange:(NSNotification *)notice
{
    DLog(@"TuneConnManager: handleNetworkChange: notice = %@", notice);
    DLog(@"TuneConnManager: handleNetworkChange: reachability status = %ld", (long)self.reachability.currentReachabilityStatus);
                     ...
                     withArgument:(NSMutableDictionary *)dict
{
    ...
    {
        DLLog(@"TuneConnManager: beginRequestGetTrackingId: %@", trackingLink);
...
- (void)beginUrlRequest:(NSString *)trackingLink encryptParams:(NSString*)encryptParams withPOSTData:(NSString*)postData
{
    ...
    NSLog(@"TuneConnManager: beginUrlRequest: URL: %@", debugString);
    NSLog(@"TuneConnManager: beginUrlRequest: post data = %@", postData);
#endif
    DLLog(@"TuneConnManager: beginUrlRequest: URL trackingLink = %@", trackingLink);
    ...
    if( [self.delegate respondsToSelector:@selector(delegate)] ) {
        ...
        if( [ddelegate respondsToSelector:@selector(_tuneSuperSecretURLTestingCallbackWithURLString:andPostDataString:)] )
            [ddelegate performSelector:@selector(_tuneSuperSecretURLTestingCallbackWithURLString:andPostDataString:) withObject:trackingLink withObject:postData];
    }
#endif
    ...
    [request setValue:[@(post.length) stringValue] forHTTPHeaderField:HTTP_CONTENT_LENGTH];
    
    DLog(@"TuneConnManager: beginUrlRequest: fire the network request.");
...
- (void)dumpQueue
{
    DLog(@"TuneConnManager: dumpQueue is already dumping queue = %d", dumpingQueue_);
    
    if( dumpingQueue_ ) return;
    if (NotReachable == self.status) return;

    DLLog(@"TuneConnManager: dumpQueue %@", self.requestsQueue );
...
- (void)dumpNextFromQueue
{
    [queueQueue addOperationWithBlock:^{
        ...
        DLLog(@"TuneConnManager: dumpNextFromQueue %@", dict);
        ...
        if (dict)
        {
            ...
            DLLog(@"TuneConnManager: dumpNextFromQueue calling beginUrlRequest: %@", [NSDate date]);
...
- (void)stopQueueDump
{
    DLLog(@"TuneConnManager: stopQueueDump");
                    ...
                    POSTData:(NSString*)postData
{
    ...
    DLLog(@"TuneConnManager: pushUrlRequestToHead %@", dict);
    ...
    DLog(@"TuneConnManager: pushUrlRequestToHead calling dumpQueue");
                  ...
                  runDate:(NSDate*)runDate
{
    ...
    DLLog(@"TuneConnManager: enqueueUrlRequest: trackingLink  = %@", trackingLink);
    ...
    DLLog(@"TuneConnManager: enqueueUrlRequest: enqueuing dict = %@", dict);
...
#pragma clang diagnostic pop
    ...
    {
        DLLog(@"TuneConnManager: connection:didFailWithError: error = %@", error);
        DLLog(@"TuneConnManager: connection:didFailWithError: failed url = %@", connection.url.absoluteString);
...
- (void)connection:(MNSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    NSLog(@"TuneConnManager: didReceiveData: connectionManager = %@", self);
    NSLog(@"TuneConnManager: didReceiveData: size = %lu, %@", (unsigned long)data.length, dataString);
#endif
    ...
    NSLog(@"TuneConnManager: didReceiveData: newCombinedData = %@", str);
#endif
}
...
- (void)connection:(MNSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    NSLog(@"TuneConnManager: didReceiveResponse: connectionManager = %@", self);
    NSLog(@"TuneConnManager: didReceiveResponse: http response code = %ld", (long)code1);
#endif
    if(!connection.isAppToApp)
    {
        ...
        if( code >= 200 && code <= 299 ) {
            ...
            DRLog(@"TuneConnManager: didReceiveResponse: previous request successful: call dumpNextFromQueue");
        ...
        // for HTTP 400, if it's from our server, drop the request and don't retry
            ...
            DRLog(@"TuneConnManager: didReceiveResponse: won't retry with 400 error" );
        }
...
{
    DLog(@"TuneConnManager: didFinishLoading: connectionManager = %@", self);

    DLLog(@"TuneConnManager: didFinishLoading: url = %@", connection.url.absoluteString);

#if DEBUG_LOG
    NSString *dataString = [[NSString alloc] initWithData:connection.responseData encoding:NSASCIIStringEncoding];
    NSLog(@"========Server Response==========");
    NSLog(@"TuneConnManager: didFinishLoading: %@", dataString);
#endif
    
    DLog(@"TuneConnManager: didFinishLoading 2: connectionManager = %@", self);
       ...
       [connection.delegateTarget respondsToSelector:connection.delegateSuccessSelector])
    {
        ...
        DLog(@"TuneConnManager: calling delegateTarget success selector: arg = %@", connection.delegateArgument);
...
#pragma clang diagnostic pop
    ...
    DLog(@"TuneConnManager: calling delegate success callback: %d", (!connection.isAppToApp && [self.delegate respondsToSelector:@selector(connectionManager:didSucceedWithData:)]));
