...
@implementation TuneCloseButton

static NSString *closeButton = @"iVBORw0KGgoAAAANSUhEUgAAAG4AAABvCAYAAAANB/VeAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAGXRFWHR"
...
- (id)init {
    if (self = [super init]) {
        ...
        NSData *data = [[NSData alloc] initWithBase64EncodedString:closeButton options:NSDataBase64DecodingIgnoreUnknownCharacters];
        UIImage *closeButtonImage = [UIImage imageWithData:data];
        
        // Set the close button's image and frame
        [self setImage:closeButtonImage];
        // Apple requires buttons to be at least 44x44
...
- (void)handleSingleTap:(UIGestureRecognizer *)recognizer {
    // Dismiss the message if close button is tapped
    [self.parentMessageView dismiss];
}
