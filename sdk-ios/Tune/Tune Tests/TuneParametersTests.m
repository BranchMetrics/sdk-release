...
@interface Tune (TuneParametersTests)
...
- (void)testAgeValid
{
    ...
    //NSLog(@"after = %f", [[NSDate date] timeIntervalSinceDate:startDate]);
...
#pragma GCC diagnostic pop
    ...
    XCTAssertTrue( [params checkDefaultValues], @"default value check failed: %@", params );
    ASSERT_KEY_VALUE( TUNE_KEY_SITE_ID, ID );
}
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    ...
    TuneSettings *sharedParams = [tune performSelector:@selector(parameters)];
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    id tune = [[Tune class] performSelector:@selector(sharedManager)];
    TuneSettings *settings = [tune performSelector:@selector(parameters)];
#pragma clang diagnostic pop
    ...
    waitFor( TUNE_SESSION_QUEUING_DELAY - iAdCallbackDelay + TUNE_TEST_NETWORK_REQUEST_DURATION );
