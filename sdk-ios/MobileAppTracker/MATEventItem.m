...
- (NSDictionary *)dictionary
{
    ...
    {
        [dict setValue:[self item] forKey:MAT_KEY_ITEM];
    }
    
    [dict setValue:[NSString stringWithFormat:@"%f", [self unitPrice]] forKey:MAT_KEY_UNIT_PRICE];
    [dict setValue:[NSString stringWithFormat:@"%d", [self quantity]] forKey:MAT_KEY_QUANTITY];
    [dict setValue:[NSString stringWithFormat:@"%f", [self revenue]] forKey:MAT_KEY_REVENUE];
    
    if([self attribute1] && [NSNull null] != (id)[self attribute1])
    {
        [dict setValue:[self attribute1] forKey:MAT_KEY_ATTRIBUTE_SUB1];
    }
    
    if([self attribute2] && [NSNull null] != (id)[self attribute2])
    {
        [dict setValue:[self attribute2] forKey:MAT_KEY_ATTRIBUTE_SUB2];
    }
    
    if([self attribute3] && [NSNull null] != (id)[self attribute3])
    {
        [dict setValue:[self attribute3] forKey:MAT_KEY_ATTRIBUTE_SUB3];
    }
    
    if([self attribute4] && [NSNull null] != (id)[self attribute4])
    {
        [dict setValue:[self attribute4] forKey:MAT_KEY_ATTRIBUTE_SUB4];
    }
    
    if([self attribute5] && [NSNull null] != (id)[self attribute5])
    {
        [dict setValue:[self attribute5] forKey:MAT_KEY_ATTRIBUTE_SUB5];
    }
