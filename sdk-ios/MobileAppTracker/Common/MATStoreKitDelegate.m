...
@interface MATStoreKitDelegate () <SKPaymentTransactionObserver>
...
static MATStoreKitDelegate *shared;


typedef void(^RequestCompletionBlock)(SKProduct *);

@interface MATStoreKitProductRequester : NSObject <SKProductsRequestDelegate>
...
@implementation MATStoreKitProductRequester
...
@implementation MATStoreKitDelegate
...
+ (void)initialize
{
    shared = [MATStoreKitDelegate new];
}
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        _products = [[NSMutableDictionary dictionary] mutableCopy];
        _productRequests = [[NSMutableDictionary dictionary] mutableCopy];
    }
    return self;
}
        ...
        switch (transaction.transactionState)
        {
            case SKPaymentTransactionStatePurchased:
            {
                ...
                if(transaction.transactionIdentifier)
                {
                    ...
                    if(product)
                    {
                        // fire a MobileAppTracker purchase request
                    ...
                    else
                    {
                        @synchronized(self.productRequests)
                        {
                            if(!self.productRequests[transaction.transactionIdentifier])
                            {
                                ...
                                MATStoreKitProductRequester *prodReq = [MATStoreKitProductRequester new];
                                ...
                                [prodReq requestProductWithId:transaction.payment.productIdentifier completion:^(SKProduct *pr){
                                    ...
                                    // fire a MobileAppTracker purchase event
...
- (void)measurePurchaseEvent:(SKPaymentTransaction *)transaction product:(SKProduct *)product receipt:(NSData *)receipt
{
    ...
    MATEventItem *eventItem = [MATEventItem eventItemWithName:productTitle unitPrice:unitPrice quantity:(int)quantity revenue:revenue];
    
    // Measure the in-app-purchase event
    MATEvent *event = [MATEvent eventWithName:@"purchase"];
